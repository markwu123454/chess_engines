...
def stockfishtime(difficulity):
    stockfish = Stockfish(path="stockfish_engine.exe", parameters={"Threads": 2, "Minimum Thinking Time": 30})
    stockfish.set_skill_level(difficulity)
    stockfish.set_fen_position(G.BOARD.fen())
    best_move = G.BOARD.parse_san(stockfish.get_best_move())
    return best_move


# Get Board Score
def calc_board_score1():
    game_status = calc_game_status()
    if game_status != "CONTINUE":
        return game_status
    ...
    w_score_p = sum([G.pawn_score1[p] for p in w_pawns])
    w_score_n = sum([G.knight_score1[p] for p in w_knights])
    w_score_b = sum([G.bishop_score1[p] for p in w_bishops])
    w_score_r = sum([G.rook_score1[p] for p in w_rooks])
    w_score_q = sum([G.queen_score1[p] for p in w_queens])
    w_score_k = sum([G.king_score1[p] for p in w_kings])
    ...
    b_score_p = sum([-G.pawn_score1[chess.square_mirror(p)] for p in b_pawns])
    b_score_n = sum([-G.knight_score1[chess.square_mirror(p)] for p in b_knights])
    b_score_b = sum([-G.bishop_score1[chess.square_mirror(p)] for p in b_bishops])
    b_score_r = sum([-G.rook_score1[chess.square_mirror(p)] for p in b_rooks])
    b_score_q = sum([-G.queen_score1[chess.square_mirror(p)] for p in b_queens])
    b_score_k = sum([-G.king_score1[chess.square_mirror(p)] for p in b_kings])
    ...
    material = (100 * pawns) + (280 * knights) + (320 * bishops)
    material = material + (479 * rooks) + (929 * queens)
    final = material + w_score_p + b_score_p + w_score_n + b_score_n
    final = final + w_score_b + b_score_b + w_score_r + b_score_r
    final = final + w_score_q + b_score_q + w_score_k + b_score_k
...
# Negamax with Alpha-Beta Pruning
def negamax_ab1(alpha, beta, depthleft):
    high_score = -9999
    if depthleft == 0:
        return calc_board_score()
